...
export default function AddToCart({
  ...
  if (variant === 'icon') {
    return (
      <button
        ...
        title={t('addToCart')}
        className="absolute bottom-3 right-3 z-10 flex h-10 w-10 items-center justify-center rounded-full bg-olive text-white shadow-[0_4px_16px_rgba(0,0,0,0.35)] transition-colors duration-200 hover:text-pink disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:text-pink"
      >
  ...
  return (
    <button
      ...
      disabled={disabled}
      onClick={add}
      className="w-full rounded-[20px] bg-olive px-8 py-4 font-display text-2xl uppercase leading-none text-white transition-colors duration-200 hover:text-pink disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:text-pink"
    >
